...
format:
  revealjs:
    ...
    show-notes: false
    smaller: true
    footer: '<a href="https://csc2626.github.io/2026F_website/" target="_blank" style="font-size:0.8em; bottom: -5px;">↩ Back to Course Website</a>'
    chalkboard:
      src: chalkboard.json
      buttons: true
      boardmarker-width: 2
    ...
    include-after-body:
      - text: |
          <script type="text/javascript" src="../mathjax-layout-fix.js"></script>
html-math-method:
  method: mathjax
...
&\text{subject to} \quad \mathbf{x}_{t+1} = f_t(\mathbf{x}_t, \mathbf{u}_t, e_t) \quad \color{red}{\text{dynamics}} \\
&\qquad \qquad \quad \mathbf{u}_t = \pi_t(\mathbf{x}_{0:t}, \mathbf{u}_{0:t-1}) \quad \color{red}{\text{control law / policy}} 
\end{aligned}
$$
:::
::::
...
$$
\begin{aligned}
&\underset{\theta}{\text{maximize}} \quad \mathbb{E}_{\tau \sim p_\theta(\tau)} \left[ \sum_{t=0}^{T} \gamma^t r(\mathbf{x}_t, \mathbf{a}_t) \right] \\
\\
&p_\theta(\tau) = p_\theta(\mathbf{x}_{0:T}, \mathbf{a}_{0:T-1}) \\
&\quad = p(\mathbf{x}_0) \prod_{t=1}^{T} \underbrace{\pi_\theta(\mathbf{a}_t | \mathbf{x}_t)}_{\color{red}{\text{policy}}} \underbrace{p(\mathbf{x}_{t+1} | \mathbf{x}_t, \mathbf{u}_t)}_{\color{red}{\text{dynamics}}}
\end{aligned}
$$
...
:::: {.column .small-math width="50%"}
$$
V^*(\mathbf{x}_t) = \max_{\mathbf{a}_t} \left[ r(\mathbf{x}_t, \mathbf{a}_t) + \gamma\mathbb{E}_{\mathbf{x}_{t+1} \sim p(\mathbf{x}_{t+1}|\mathbf{x}_t, \mathbf{a}_t)} \left[ V^*(\mathbf{x}_{t+1}) \right] \right]
$$
...
“if you land at state x and you follow the optimal actions/policy
...
:::: {.column .small-math width="50%"}
$$
V^*(\mathbf{x}_t) = \max_{\mathbf{a}_t} \underbrace{\left[ r(\mathbf{x}_t, \mathbf{a}_t) + \gamma\mathbb{E}_{\mathbf{x}_{t+1} \sim p(\mathbf{x}_{t+1}|\mathbf{x}_t, \mathbf{a}_t)} \left[ V^*(\mathbf{x}_{t+1}) \right] \right]}_{Q^*(\mathbf{x}_t, \mathbf{a}_t)}
$$
...
“if you land at state $\mathbf{x}_t$, and you commit\
to first execute action $\mathbf{a}_t$, and then\
follow the optimal actions/policy, how much\
...
Value function of policy $\pi$: “if you land at state $x_t$ and you follow policy $\pi$ what is the expected reward you will accumulate?”\
$\downarrow$
:::

$$
V^\pi(\mathbf{x}_t) = \mathbb{E}_{\mathbf{a}_t \sim \pi(\mathbf{a}|\mathbf{x}_t)} \left[ r(\mathbf{x}_t, \mathbf{a}_t) + \gamma\mathbb{E}_{\mathbf{x}_{t+1} \sim p(\mathbf{x}_{t+1}|\mathbf{x}_t, \mathbf{a}_t)} \left[ V^\pi(\mathbf{x}_{t+1}) \right] \right]
$$
::::
:::::::
...
$$
V^\pi(\mathbf{x}_t) = \mathbb{E}_{\mathbf{a}_t \sim \pi(\mathbf{a}|\mathbf{x}_t)} \overbrace{\left[ r(\mathbf{x}_t, \mathbf{a}_t) + \gamma\mathbb{E}_{\mathbf{x}_{t+1} \sim p(\mathbf{x}_{t+1}|\mathbf{x}_t, \mathbf{a}_t)} \left[ V^\pi(\mathbf{x}_{t+1}) \right] \right]}^{Q^{\pi} (\textbf{x}_t, \textbf{a}_t)}
$$
::::
:::::::

::: {.red-annotation style="position: absolute; bottom: 180px; right: 0;"}
State-action value function of policy $\pi$:\
“if you land at state $\textbf{x}_t$, and you commit\
to first execute action $\textbf{a}_t$, and then follow\
policy $\pi$ how much reward will you\
accumulate in expectation?” $\downarrow$
:::
...
::::: {.column .small-math width="50%"}
$$
V^*(\mathbf{x}_t) = \max_{\mathbf{a}_t} \underbrace{\left[ r(\mathbf{x}_t, \mathbf{a}_t) + \gamma\mathbb{E}_{\mathbf{x}_{t+1} \sim p(\mathbf{x}_{t+1}|\mathbf{x}_t, \mathbf{a}_t)} \left[ V^*(\mathbf{x}_{t+1}) \right] \right]}_{Q^*(x_t, a_t)}
$$
...
$$
\begin{align}
u_0^*, \ldots, u_{N-1}^* &= \text{argmin}_{u_0, \ldots, u_N} \sum_{t=0}^{N} c(\mathbf{x}_t, \mathbf{u}_t) \\
&\text{s.t.} \\
...
• $J_0(x) = x^TQx$ is the terminal cost of ending up at state $x$, with no actions left to perform. Recall that $c(x, u) = x^TQx + \cancel{u^T R u}$
...
For notational convenience later on, let's define $P_0 = Q$
:::
::::
...
::: vertical-align
$$\begin{align}
J_1(\mathbf{x}) &= \min_{\mathbf{u}} \underbrace{[\mathbf{x}^T Q \mathbf{x} + \mathbf{u}^T R \mathbf{u} + J_0(A\mathbf{x} + B\mathbf{u})]}_{\color{red}\text{In RL this would be the state-action value function}} \\
\end{align}$$
...
\begin{align}
J_0(\mathbf{x}) &= \mathbf{x}^TP_0\mathbf{x} \\
...
$\color{red}\uparrow$\
[Q: Is this matrix invertible? Recall that $R$, $P_0$ are positive definite matrices.]{.red-annotation}
...
$\frac{\partial}{\partial \mathbf{u}}(\mathbf{u}^T M \mathbf{b}) = M^T \mathbf{b}$
...
::: {.column width="40%"}
$P_0 = Q$\
// n is the number of steps left
:::

::: {.column .red-annotation .fragment width="60%" fragment-index="3"}
Potential problem for controls $\mathbf{u}$ of dimension $k$ \>\> 100:\
...
::: {.column .red-annotation .fragment width="70%" fragment-index="1"}
...
::::: columns
::: {.column width="80%"}
Optimal control parameters for time t = N – n are computed backwards in time: $\mathbf{u}_t = K_t \mathbf{x}_t$ with cost-to-go $J_t(\mathbf{x}) = \mathbf{x}^T P_t \mathbf{x}$, whereas the states are predicted forward in time according to linear dynamics
:::

::: {.column .red-annotation .fragment width="70%" fragment-index="2"}
One pass **forward** in time: predict states, compute controls and cost-to-go
:::
:::::
...
Linearize dynamics around that initial sequence:

::: small-math
$$
f(x_t, u_t) \approx \underbrace{f(\bar{x}_t, \bar{u}_t)}_{\mathbf{b}_t} + \underbrace{\frac{\partial f}{\partial x} (\bar{x}_t, \bar{u}_t)}_{A_t}\underbrace{(x_t - \bar{x}_t)}_{\delta \mathbf{x}_t} + \underbrace{\frac{\partial f}{\partial u} (\bar{x}_t, \bar{u}_t)}_{B_t}\underbrace{(u_t - \bar{u}_t)}_{\delta \mathbf{u}_t} = \tilde{f}(\delta \mathbf{x}_t, \delta \mathbf{u}_t)
$$
:::

. . .

Do a Taylor expansion of the cost:

::: small-math
$$
c(x_t, u_t) \approx c(\bar{x}_t, \bar{u}_t) + \underbrace{\nabla_{x,u} c(\bar{x}_t, \bar{u}_t)}_{\mathbf{h}_t}
\begin{bmatrix}
\delta \mathbf{x}_t \\
\delta \mathbf{u}_t
\end{bmatrix}
...
\delta \mathbf{x}_t \\
\delta \mathbf{u}_t
\end{bmatrix} = \tilde{c}(\delta \mathbf{x}_t, \delta \mathbf{u}_t) 
$$
:::

. . .

Apply LQR backward pass on the approximate dynamics $\tilde{f}(\delta \mathbf{x}_t, \delta \mathbf{u}_t)$ and cost $\tilde{c}(\delta \mathbf{x}_t, \delta \mathbf{u}_t)$ to compute controller and cost-to-go parameters.

. . .

Do an LQR forward pass to get $\delta \mathbf{u}_t$ and $\delta \mathbf{x}_t$ and update state and action sequence and $\bar{x}_0, \ldots, \bar{x}_N$ and $\bar{u}_0, \ldots, \bar{u}_N$
...
{{< video https://www.youtube.com/watch?v=RHnXD6uO3Mg width="70%" height="50%" >}}

[<https://www.youtube.com/watch?v=RHnXD6uO3Mg>]{.small-math}

## Online trajectory optimization / MPC


{{< video https://www.youtube.com/watch?v=anIsw2-Lbco width="70%" height="50%" >}}

[<https://www.youtube.com/watch?v=anIsw2-Lbco>]{.small-math}


## Online trajectory optimization / MPC

{{< video https://www.youtube.com/watch?v=bjlT-6KVQ7U width="70%" height="50%" >}}

[<https://www.youtube.com/watch?v=bjlT-6KVQ7U>]{.small-math}
...
## How do we plan through a learned model?

::: hanging-bullets
- iLQR needs $\frac{\partial f}{\partial x}, \frac{\partial f}{\partial u}$ and $\nabla^2 c$: the model and the cost must be smooth, and the Hessian should be positive definite


Often that is not the case:

- $f_\theta$ is a neural network (gradients exist but are noisy, and the linearization can be very poor far from the data)

- $f$ is a black-box simulator with contacts (no gradients at all)

- $c$ is discontinuous, e.g. an indicator for collisions or reaching a goal region
:::

. . .

[Idea: don’t differentiate at all. Sample many action sequences, roll them out through the model in parallel, and average the good ones]{style="color:red;"}

## Model Predictive Path Integral Control (MPPI)

Given the current state $x_0$ and a nominal action sequence $\bar{u}_0, \ldots, \bar{u}_{N-1}$ (e.g. last plan, shifted by one step):

::: small-math
1.  Sample $K$ perturbed sequences: $\epsilon^k_t \sim \mathcal{N}(0, \Sigma)$, $\quad u^k_t = \bar{u}_t + \epsilon^k_t$

2.  Roll out each one through the model: $x^k_{t+1} = f(x^k_t, u^k_t)$, starting from $x_0$

3.  Compute the cost of each rollout: $S_k = \sum_{t=0}^{N-1} c(x^k_t, u^k_t) + c_N(x^k_N)$

4.  Weight rollouts with a softmin over costs (temperature $\lambda$):
    $$
    w_k = \frac{\exp\left(-\frac{1}{\lambda}\left(S_k - \min_j S_j\right)\right)}{\sum_{j=1}^{K} \exp\left(-\frac{1}{\lambda}\left(S_j - \min_j S_j\right)\right)}
    $$

5.  Update the nominal sequence: $\bar{u}_t \leftarrow \bar{u}_t + \sum_{k=1}^{K} w_k \, \epsilon^k_t$

6.  Execute $\bar{u}_0$, observe the new state, shift the sequence, repeat (MPC)
:::

. . .

[Steps 1–3 are $K$ independent rollouts, so they map perfectly onto a GPU: thousands of rollouts at 40+ Hz on a real vehicle]{.red-annotation}

## MPPI in action: reaching a goal

::::: columns
::: {.column width="65%"}
![](images/mppi-reaching.gif){width="100%"}
:::

::: {.column width="35%"}
::: small-math
Point mass with friction: $m\ddot{p} = u - \alpha \dot{p}$

Cost: $c(x, u) = \|p - p_{\text{goal}}\|^2_{Q} + \|\dot{p}\|^2 + \|u\|^2_R$

At every step:

• Blue: the $K$ sampled rollouts, opacity $\propto w_k$

• Red: the new nominal plan $\bar{u} + \sum_k w_k \epsilon^k$

• Black: the executed path (only $\bar{u}_0$ is applied, then replan)
:::

::: red-annotation
Notice how the samples are spread out when far from the goal and collapse around it once we arrive: the weights and the warm start do the work, no gradients needed
:::
:::
:::::

## MPPI: properties & tricks

::: hanging-bullets
- $\lambda \to 0$: only keep the best rollout (greedy). $\lambda \to \infty$: plain average of all samples. In between: a smoothed, weighted average of the good samples

- Derivative-free: works with any model you can evaluate, including neural networks and simulators with contact, and with discontinuous costs

- Warm start is essential: reuse last plan shifted by one step. With a good warm start, one MPPI update per control step is often enough

- Problems: $K$ needs to grow with the action dimension and horizon; exploration noise $\Sigma$ has to be tuned; no local convergence guarantee like iLQR
:::

::: red-annotation
See “Model Predictive Path Integral Control: From Theory to Parallel Computation” 

“Information Theoretic MPC for Model-Based Reinforcement Learning” by Williams, Aldrich, Theodorou et al. (2017),

where MPPI is used to drive the AutoRally car aggressively with a learned neural network dynamics model
:::

## A closely related idea: the Cross-Entropy Method (CEM)

• MPPI keeps a single nominal sequence $\bar{u}$ and nudges it with a weighted average of the noise

. . .

• CEM instead maintains a *distribution* over action sequences, $q(u_{0:N-1}) = \mathcal{N}(\mu, \Sigma)$, and iteratively refits it to the best samples

. . .

• Comes from rare-event simulation (Rubinstein, 1997): to estimate the probability of a rare event, iteratively move the sampling distribution towards the region where the event happens

. . .

[For optimization the “rare event” is “cost below the current threshold”, and the distribution concentrates on low-cost action sequences]{style="color:red;"}

## Cross-Entropy Method (CEM) for planning

Given the current state $x_0$ and an initial distribution $\mu_t, \Sigma_t$ over actions (e.g. $\mu$ = last plan shifted by one step, $\Sigma$ reset to a large value):

::: small-math
Repeat for a few iterations:

1.  Sample $K$ action sequences: $u^k_{0:N-1} \sim \mathcal{N}(\mu_{0:N-1}, \Sigma_{0:N-1})$

2.  Roll out each one through the model: $x^k_{t+1} = f(x^k_t, u^k_t)$, starting from $x_0$

3.  Compute the cost of each rollout: $S_k = \sum_{t=0}^{N-1} c(x^k_t, u^k_t) + c_N(x^k_N)$

4.  Keep the $M \ll K$ lowest-cost sequences, the *elites* $\mathcal{E}$

5.  Refit the distribution to the elites:
    $$
    \mu_t \leftarrow \frac{1}{M} \sum_{k \in \mathcal{E}} u^k_t \qquad \Sigma_t \leftarrow \frac{1}{M} \sum_{k \in \mathcal{E}} (u^k_t - \mu_t)(u^k_t - \mu_t)^T
    $$

Execute $\mu_0$, observe the new state, shift $\mu$, repeat (MPC)
:::

. . .

[Step 5 is a maximum-likelihood fit of a Gaussian to the elites, i.e. it minimizes the cross-entropy between the elite distribution and $q$, hence the name]{.red-annotation}

## CEM in action: reaching a goal

::::: columns
::: {.column width="65%"}
![](images/cem-reaching.gif){width="100%"}
:::

::: {.column width="35%"}
::: small-math
Same point mass and reaching cost as in the MPPI example

Each frame is one CEM iteration; 3 iterations per control step

• Blue: the $K$ samples from $\mathcal{N}(\mu, \Sigma)$

• Orange: the $M$ elites

• Red: the new mean plan $\mu$, fit to the elites

• Black: the executed path (only $\mu_0$ is applied, then replan)
:::

::: red-annotation
Watch the sample cloud shrink within each control step as $\Sigma$ is refit to the elites, then widen again when $\Sigma$ is reset for the next step
:::
:::
:::::
...
|                    | CEM                                                          | MPPI                                                         |
|--------------------|--------------------------------------------------------------|--------------------------------------------------------------|
| What is updated    | mean $\mu$ **and** covariance $\Sigma$                       | nominal sequence $\bar{u}$ only ($\Sigma$ fixed)             |
| How samples are used | hard threshold: top $M$ elites, equal weight              | soft weights $w_k \propto \exp(-S_k / \lambda)$ on all samples |
| Iterations per step | usually several (distribution needs to shrink)              | often one, thanks to warm start                              |
| Hyperparameters    | $K$, $M$, initial $\Sigma$, # iterations                     | $K$, $\lambda$, $\Sigma$                                     |
| Behavior           | more exploitative, converges quickly, can collapse too early | smoother updates, more robust to noisy costs                 |
:::

. . .

::: hanging-bullets
- Both are derivative-free, embarrassingly parallel, and work with learned models and discontinuous costs

- Both scale poorly with action dimension and horizon: a Gaussian over $N \cdot \dim(u)$ variables gets very wide, very fast

- Tricks: keep the top elites across iterations, smooth $\mu$ over time (colored noise), sample in a low-dimensional space of action splines
:::

::: red-annotation
CEM was the planner of choice in many model-based RL papers, e.g. “Deep RL in a Handful of Trials using Probabilistic Dynamics Models” (PETS, Chua et al. 2018) and “Learning Latent Dynamics for Planning from Pixels” (PlaNet, Hafner et al. 2019)
:::
...
[• Model Predictive Path Integral Control (MPPI)]{style="color:grey;"}\
[• Cross-Entropy Method (CEM)]{style="color:grey;"}\
• Appendix
:::
